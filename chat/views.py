...
def inbox(request, template_name='django_messages/inbox.html'):
    ...
    '''
    message_in = Message.objects.inbox_for(request.user)
    message_out = Message.objects.outbox_for(request.user)
    context = {
...
class Chat(UpdateView):
    ...
    def get_success_url(self):
        return reverse('inbox', kwargs={'pk': self.object.pk})
    ...
    def get_initial(self, *args, **kwargs):
        recipient = self.get_object(queryset=Appointment.objects.select_related('appointment.user.id'))
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            recipient = self.get_object(queryset=Appointment.objects.select_related('appointment.user.id'))
            #user = User.objects.all().get(id=recipient.appointment.user.id)
...
@login_required
def compose(request, form_class=MyComposeForm,
...
@login_required
def reply(request, message_id, form_class=MyComposeForm,
        template_name='django_messages/compose.html', success_url=None,
...
@login_required
def view(request, message_id, form_class=MyComposeForm, quote_helper=format_quote,
        subject_template=_(u"Re: %(subject)s"),
